...
class KnowledgeRepository @Inject constructor(private val database: VaultDatabase, private val vault: VaultRepository) {
    ...
    suspend fun getRelationships(itemId: String): List<com.vaultbrain.core.database.entity.RelationshipEntity> {
        if (DecoySessionState.isDecoy.value) return emptyList()
        val result = database.knowledgeGraphDao().getRelationshipsForItem(itemId)
        return if (DecoySessionState.isDecoy.value) emptyList() else result
    }

    suspend fun relationshipsForItems(itemIds: List<String>): List<com.vaultbrain.core.database.entity.RelationshipEntity> {
        if (DecoySessionState.isDecoy.value) return emptyList()
        val result = database.knowledgeGraphDao().getRelationshipsForItems(itemIds)
        return if (DecoySessionState.isDecoy.value) emptyList() else result
    }

    suspend fun related(itemId: String): List<com.vaultbrain.core.common.model.VaultItem> {
    ...
    suspend fun replace(itemId: String, facts: List<DerivedFactEntity>) {
        ...
        if (item.isArchived || item.isStealth || item.enrichmentState == com.vaultbrain.core.common.model.EnrichmentState.SKIPPED_PRIVACY) return
    ...
    suspend fun replaceRelationships(itemId: String, relationships: List<com.vaultbrain.core.database.entity.RelationshipEntity>) {
        if (DecoySessionState.isDecoy.value) return
        val item = vault.getById(itemId) ?: return
        if (DecoySessionState.isDecoy.value) return
        if (item.isArchived || item.isStealth || item.enrichmentState == com.vaultbrain.core.common.model.EnrichmentState.SKIPPED_PRIVACY) return
        val source = item.rawOcrText.orEmpty()
        val validTypes = com.vaultbrain.core.common.model.RelationshipType.entries.map { it.name }.toSet()
