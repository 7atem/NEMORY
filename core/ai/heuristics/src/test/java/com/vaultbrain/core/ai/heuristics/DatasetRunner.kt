...
fun main() {
    ...
    println("# Dataset Extraction Results")
    datasets.forEach { (name, text) ->
        val res = extractor.extract(text, emptyList(), null, 0f)
        println("## " + name)
        println("- **Lens**: " + res.lensTags.joinToString())
        println("- **Classification**: " + res.inferredClassification)
        println("- **Metadata**: " + res.metadata)
        if (res.alertDate != null) println("- **Alert Scheduled**: Yes (Timestamp: " + res.alertDate + ")")
        println()
    }
}
