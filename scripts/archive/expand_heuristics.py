...
content = re.sub(r'        private val TAX_PATTERN', new_patterns + r'        private val TAX_PATTERN', content)

# Enhance buildMetadata
new_metadata = '''
            when (classification) {
...
# We need to replace the entire when (classification) { block inside uildMetadata
content = re.sub(r'            when \(classification\) \{[\s\S]*?else -> Unit\s*\}', new_metadata, content)

# Enhance parseDates for Proactivity
new_parse_dates = '''
...
content = re.sub(r'        val now = System\.currentTimeMillis\(\)[\s\S]*?else -> null\s*\}', new_parse_dates, content)

# Also need to add 	ext: String as a parameter to parseDates to find explicit text patterns like DUE_DATE
content = re.sub(r'parseDates\(dates: List<String>, classification: Classification\)', 'parseDates(dates: List<String>, classification: Classification, text: String)', content)
content = re.sub(r'parseDates\(dates, fusedClassification\)', 'parseDates(dates, fusedClassification, text)', content)
